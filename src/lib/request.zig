...
pub usingnamespace struct {
    ...
    pub const Sender = struct {
        ...
        fn toBytes(self: *Sender) !?[]const u8 {
            var buffer = try Buffer.create(self.request.allocator, self.request.parts.method.toString());
            defer buffer.deinit();
            try buffer.concat(" ");
            // try buffer.concat("https://");
            // try buffer.concat(self.request.parts.uri.host.?);
            try buffer.concat(self.request.parts.uri.path);
            try buffer.concat(" ");
            try buffer.concat(self.request.parts.version.toString());
            try buffer.concat("\r\n");
            var it = self.request.parts.headers.iterator();
            while (it.next()) |header| {
                try buffer.concat(header.name);
                try buffer.concat(": ");
                try buffer.concat(header.value);
                try buffer.concat("\r\n");
            }
            ...
            if (self.request.parts.method.shouldHaveBody()) {
                // try buffer.concat(self.request.body.buffer);
            }

            const data = try buffer.toOwned();
            if (data) |bytes| {
                return bytes;
            } else {
                return null;
...
test "request" {
    ...
    request.body = Body.new("{ \"name\":  \"oooop\", \"age\": 54 }");
